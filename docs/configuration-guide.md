...
效果：
- 开启：在右侧固定面板显示当前可见实体的相关日志流。
- 关闭：不显示右侧日志面板。

特点：
- 每行以 `#EntityId` 开头，方便和场上短标签对应。
- 长日志会自动换行完整显示，不再用 `...` 截断。
- 适合实体多、实体旁日志互相重叠、事后看录屏的场景。
...
收到 `19 Death`、`27 HP=0`、`105 CurrentHP=0`、`04 RemoveCombatant` 或 `105:Remove` 时，该实体会立即隐藏；没有这些终止事件、但也没有后续日志活动时，会等生命超时后隐藏。
